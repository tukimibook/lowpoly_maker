...
overview: 【Google Play へのリリース予定アプリ】品質優先（破綻しない・シンプルなコード）。頂点溶接モデル・座標継ぎ目・v1芯は下絵+なぞり+ズーム+テッセレーション+保存。Phase A〜E+・G-spike・Hα・Hβ・F・G 完了。図形タップ選択（Phase Select）を Hγ 前／並行へ前倒し。次は Select∥Hγ→Hδ→R。
todos:
  ...
  - id: phase-select-hit-test
    content: "Phase Select(Hγ前/並行): 図形タップ選択(Hit-Testing)。AABB事前フィルタ+Point-in-Polygon、Z-Order最前面優先、頂点ヒットとの優先順位固定。選択後は既存辺トグル連携。彩色の最小入口(選択図形の塗り+Undo)。詳細は plan_future_phases.md"
    status: pending
  - id: phase-h-rest
    content: "Phase Hγ(着手中)/Hδ: 保存(JSON+schemaVersion+下絵コピー+自動保存+作品一覧)+PNGエクスポート。以降: Redo永続化・グリッド・SVG・スポイト・設定・仕上げはv1.1以降（図形タップ選択は Phase Select へ前倒し済み）"
    status: pending
  - id: phase-r-release
...
> - [.cursor/plans/plan_phase_H_gamma.md](.cursor/plans/plan_phase_H_gamma.md): **現在着手中の Phase（Hγ: 保存・作品一覧）** の詳細仕様・着手前チェックリスト・関連テスト・直近の検討メモ。着手フェーズが進んだら、次フェーズの詳細に中身を差し替え、**ファイル名も次フェーズ名にリネーム**して使い続ける運用とする（例: `plan_phase_G.md` → `plan_phase_H_gamma.md`、2026-07-20 実施）。
> - [.cursor/plans/plan_future_phases.md](.cursor/plans/plan_future_phases.md): 未着手フェーズ（Hδ/R）の詳細仕様、コード品質・技術的負債表、テスト方針、リスクと対策。
> - [.cursor/plans/plan_archive_history.md](.cursor/plans/plan_archive_history.md): 完了済みフェーズ（A〜E+、G-spike、Hα、Hβ、F、G）の実装済み仕様と、過去（2026-07-10〜07-20）の検討メモアーカイブ（2026-07-17 新設、コンテキスト肥大化防止のため `plan_future_phases.md` から分離）。
>
> **運用ルール（継続）**: 議論の経緯・決定理由は該当ファイル末尾の「検討メモ」に日付付きで積み上げる（直近は `plan_phase_H_gamma.md`、過去は `plan_archive_history.md` が正本）。単純な情報追加は各 Phase の節に直接追記してよい。既存の決定そのものを明確に覆す場合のみ、その行を削除せず ~~取り消し線~~ を引いて残し、検討メモの該当日付エントリを参照させる。関連する検討メモがある節には「→ 検討メモ（日付、ファイル名）参照」の一行リンクを添える。
>
> **着手用サマリ**: [.cursor/plans/plan_phase_H_gamma.md](.cursor/plans/plan_phase_H_gamma.md) 冒頭の **現在のステータス** に Phase・完了条件・次ステップを置く。新チャット・AI 着手時は `@plan_phase_H_gamma.md` を付ける。Phase 完了コミット後に **現在のステータス** を更新。仕様の詳細は `plan_phase_H_gamma.md`（現在フェーズ）／`plan_future_phases.md`（未着手フェーズ）／`plan_archive_history.md`（完了済みフェーズ・過去ログ）が正本、本ファイルは全体地図。frontmatter の `overview`（先頭 YAML 4行目付近）は Plan 用1行要約。
...
flowchart TD
  ...
  G --> Select["Phase Select 図形タップ選択 / Hit-Testing"]
  Select --> Hg["Phase Hγ 保存・作品一覧"]
  G -.->|並行可| Hg
  Select -.->|並行可| Hg
...
**並行メモ**: G-spike は Hα より前に **0.5〜2日** で de-risk。**完了済み（2026-07-15、Go・Tier B確定）**。Hα・Hβ・F・G 完了。**Phase Select（図形タップ選択）を Hγ 前／並行へ前倒し（2026-07-27）**。Phase R の applicationId・署名・プライバシーは Hγ 前後に並行推奨。
