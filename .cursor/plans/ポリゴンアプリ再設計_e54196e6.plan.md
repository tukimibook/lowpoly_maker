...
overview: 【Google Play へのリリース予定アプリ】品質優先（破綻しない・シンプルなコード）。頂点溶接モデル・座標継ぎ目・v1芯は下絵+なぞり+ズーム+テッセレーション+保存+図形選択/彩色+PNG。Phase A〜E+・G-spike・Hα・Hβ・F・G・Hγ・Select・Hδ 完了。次は Phase R（ストア公開準備）。
todos:
  ...
  - id: phase-select-hit-test
    ...
    status: completed
  - id: phase-h-rest
    content: "Phase Hγ(完了)/Hδ(完了): 保存・PNG エクスポート完了。詳細は plan_archive_history.md。以降: Redo永続化・グリッド・SVG・スポイト・設定・仕上げはv1.1以降"
    status: completed
  - id: phase-r-release
    content: "Phase R(着手中): ストア公開準備+統合smokeフル実行(U2)。applicationID・署名・プライバシー・UMP・AdMob・QA。詳細は plan_phase_R.md。リワード解放フックは後付け"
    status: pending
isProject: false
---
...
> - [.cursor/plans/plan_phase_R.md](.cursor/plans/plan_phase_R.md): **現在着手中の Phase（R: ストア公開準備）** の詳細仕様・着手前チェックリスト・関連テスト・直近の検討メモ。着手フェーズが進んだら、次フェーズの詳細に中身を差し替え、**ファイル名も次フェーズ名にリネーム**して使い続ける運用とする（例: `plan_phase_H_delta.md` → `plan_phase_R.md`、2026-08-14 実施）。
> - [.cursor/plans/plan_future_phases.md](.cursor/plans/plan_future_phases.md): 技術的負債表、テスト方針、リスクと対策。v1.1 以降の要約。
> - [.cursor/plans/plan_archive_history.md](.cursor/plans/plan_archive_history.md): 完了済みフェーズ（A〜E+、G-spike、Hα、Hβ、F、G、Hγ、Select、Hδ）の実装済み仕様と、過去の検討メモアーカイブ（2026-07-17 新設、コンテキスト肥大化防止のため `plan_future_phases.md` から分離）。
>
> **運用ルール（継続）**: 議論の経緯・決定理由は該当ファイル末尾の「検討メモ」に日付付きで積み上げる（直近は `plan_phase_R.md`、過去は `plan_archive_history.md` が正本）。単純な情報追加は各 Phase の節に直接追記してよい。既存の決定そのものを明確に覆す場合のみ、その行を削除せず ~~取り消し線~~ を引いて残し、検討メモの該当日付エントリを参照させる。関連する検討メモがある節には「→ 検討メモ（日付、ファイル名）参照」の一行リンクを添える。
>
> **着手用サマリ**: [.cursor/plans/plan_phase_R.md](.cursor/plans/plan_phase_R.md) 冒頭の **現在のステータス** に Phase・完了条件・次ステップを置く。新チャット・AI 着手時は `@plan_phase_R.md` を付ける。Phase 完了コミット後に **現在のステータス** を更新。仕様の詳細は `plan_phase_R.md`（現在フェーズ）／`plan_future_phases.md`（負債表・v1.1 要約）／`plan_archive_history.md`（完了済みフェーズ・過去ログ）が正本、本ファイルは全体地図。frontmatter の `overview`（先頭 YAML 4行目付近）は Plan 用1行要約。
...
flowchart TD
  ...
  Select --> Hd["Phase Hδ PNG エクスポート ✅"]
  Hd --> R["Phase R ストア公開準備（着手中）"]
  R -.->|並行可| Rearly["applicationId・署名・プライバシー"]
```

**並行メモ**: G-spike は Hα より前に **0.5〜2日** で de-risk。**完了済み（2026-07-15、Go・Tier B確定）**。Hα・Hβ・F・G・Hγ・Select・**Hδ 完了（2026-08-10）**。追加 UI/UX 記録は 2026-08-14。Phase R を現在着手中へ引き込み。
...
| Hδ | なし（出力はレンダ） | 中（エクスポート経路） | なし | 是 |
| R | なし | 小（ID/広告/法務） | なし | 是 |
...
- 保存スロット/エクスポートの方針:
  - 無料の保存枚数は ~~10 枚~~ **8 枚**。上限到達時はギャラリーで「消して空ける」。リリース後にリワードでスロット追加。→ 検討メモ（2026-08-16、[plan_phase_R.md](plan_phase_R.md)）参照。
