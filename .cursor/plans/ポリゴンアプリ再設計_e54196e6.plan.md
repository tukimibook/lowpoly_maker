...
overview: 【Google Play へのリリース予定アプリ】品質優先（破綻しない・シンプルなコード）。頂点溶接モデル・座標継ぎ目・v1芯は下絵+なぞり+ズーム+テッセレーション+保存。Phase A〜E+・G-spike・Hα・Hβ・F・G 完了。次は Hγ→Hδ→R。
todos:
  ...
  - id: phase-g-tessellation
    content: "Phase G: 自動テッセレーション(三角・maxEdge/minEdge固定・UI1ボタン)。#6/#8/#10/#17/#20すべて完了。maxEdge/minEdgeは実機チューニングで150.0/25.0に確定。粗さスライダーはv1.1"
    status: completed
  - id: phase-h-rest
    content: "Phase Hγ(着手中)/Hδ: 保存(JSON+schemaVersion+下絵コピー+自動保存+作品一覧)+PNGエクスポート。以降: Redo永続化・グリッド・SVG・スポイト・設定・仕上げはv1.1以降"
    status: pending
  - id: phase-r-release
...
**並行メモ**: G-spike は Hα より前に **0.5〜2日** で de-risk。**完了済み（2026-07-15、Go・Tier B確定）**。Hα・Hβ・F も完了済み（2026-07-19）、現在は Phase G（自動テッセレーション）着手中。Phase R の applicationId・署名・プライバシーは Hγ 前後に並行推奨。
