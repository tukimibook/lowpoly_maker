...
overview: 【Google Play へのリリース予定アプリ】品質優先（破綻しない・シンプルなコード）。頂点溶接モデル・座標継ぎ目・v1芯は下絵+なぞり+ズーム+テッセレーション+保存。Phase A〜E+ 完了。次は G-spike→Hα→Hβ→F→G→Hγ→Hδ→R。
todos:
  ...
  - id: phase-g-tessellation
    content: "Phase G: 自動テッセレーション(三角・maxEdge/minEdge固定・UI1ボタン)。実装前にmaxEdge/minEdgeのworld値を相談で確定。compute(Isolate)+ローディング(#17)。#6/#8/#10/#17/#20。粗さスライダーはv1.1"
    status: pending
  - id: phase-h-rest
    content: "Phase Hγ/Hδ: 保存(JSON+schemaVersion+下絵コピー+自動保存+作品一覧)+PNGエクスポート。以降: Redo永続化・グリッド・SVG・スポイト・設定・仕上げはv1.1以降"
    status: pending
  - id: phase-r-release
...
> リリース予定: 本アプリは **Google Play で正式リリース予定**。したがって「とりあえず動く」ではなく、拡張性・保守性・製品品質（下記「リリース要件」「品質方針」「コード品質・修正前提」）を満たす前提で実装する。収益化は **広告のみ**（v1 はバナー最小、リリース後にリワードで解放）。**課金（IAP）は現時点では実装しないが、将来追加できる可能性は残す**（エンタイトルメントの継ぎ目を汎用に作る）。
...
>
> **現行サマリ（着手用）**: 同フォルダの [.cursor/plans/現行サマリ.md](.cursor/plans/現行サマリ.md) に **今の Phase・完了条件・直近決定** だけを短く置く。新チャット・AI 着手時はこちらを `@` する。Phase 完了コミット後に更新。仕様の詳細は本ファイルが正本。frontmatter の `overview`（先頭 YAML 4行目付近）は Plan 用1行要約。
...
- **Phase 完了条件**: `flutter analyze` / `flutter test` に加え **実機確認**、および下記「コード品質・修正前提」の該当チェック項目を満たすこと（→ 検討メモ 2026-07-13 参照）。
- **コミット**: Phase 完了＝1コミット（実機OK後）。同一ファイルに複数 Phase が載るときは無理に分割しない（→「Git / コミット運用」節）。

## 戻りを最小化する実装順序

体験が早く揃う順。下絵＋ズームは G より先（「写真をなぞってローポリ化」が v1 の芯）。→ 検討メモ（2026-07-13）参照。
...
flowchart TD
  ...
  Hg --> Hd["Phase Hδ PNG エクスポート"]
  Hd --> R["Phase R ストア公開準備"]
  R -.->|並行可| Rearly["applicationId・署名・プライバシー"]
```

**並行メモ**: G-spike は Hα より前に **0.5〜2日** で de-risk。Phase R の applicationId・署名・プライバシーは Hγ 前後に並行推奨。

**v1.1 以降（初回見送り）**: G 粗さスライダー、スポイト、グリッド、SVG、リワード広告、Redo 永続化、下絵自由変形、設定画面本格版、豪華チュートリアル。
...
> 一言: **写真やイラストを下絵にして、拡大しながらなぞり、自動で三角ローポリにし、編集・保存・PNG共有できる Android アプリ**。→ 検討メモ（2026-07-13）参照。
...
- G 粗さスライダー、スポイト（下絵から色取得）、グリッド、SVG、高解像度/透かし、リワード広告、Redo/Undo 再起動後保持、下絵の自由変形、設定画面本格版、IAP（エンタイトルメント継ぎ目のみ v1 で用意）、豪華チュートリアル
...
> **未カバー（Hβ で対応）**: 座標**数学**の継ぎ目のみ。ジェスチャー（pan vs ピンチズーム）は Phase C で `onPan*` を導入したため、ズーム UI 追加時に `polygon_canvas` の書き直しが必要（→ コード品質節 #2、検討メモ 2026-07-13）。
...
> **目的**: G 直前に集中していた小さな修正を前倒しし、「G前ゲート」の崖を緩める。→ 検討メモ（2026-07-13 追記続き2）参照。
...
→ 検討メモ（2026-07-14 追記続き5）参照。

## Phase G-spike: テッセレーション実現性検証

> v1 の**目玉**（G）の de-risk。本番実装前に **0.5〜2日** で検証。Hα より前に実施。→ 検討メモ（2026-07-13 追記続き2・続き3・2026-07-14 追記続き4）参照。
...
- **三角サイズ品質（#20）**: spike では **仮の** `maxEdge` で「辺長が max を超える三角形が残らないか」を Pass 条件の1つとする。**具体的な world 値（物理サイズ）は現時点では未確定** — Hβ・F 実機後、**G 本番着手前に相談して確定**する（→ 検討メモ 2026-07-14 追記続き4）。
- **Go/No-Go と Tier 出口**:
...
> **着手前提（必須）**: 下記「コード品質・修正前提」の F-core / F-UI チェック。`handleDrawTap` の単純再利用はしない。→ 検討メモ（2026-07-13）参照。
...
> G より先に実装（v1 の芯）。保存（Hγ）と同時リリース — セッションのみの下絵は不可。→ 検討メモ（2026-07-13・追記続き3）参照。
...
> **着手前提（必須）**: 許容距離の画面 px 統一（下記コード品質節）。ジェスチャー方針を先に決める。→ 検討メモ（2026-07-13）参照。
...
> **着手前提（必須）**: G-spike 完了 + 下記コード品質節の G 本番直前チェック + **maxEdge/minEdge の world 値を実装前に相談で確定**（#20）。→ 検討メモ（2026-07-13・2026-07-14 追記続き4）参照。
...
~~既存計画の以下を **G の後に一括接続**: グリッド表示/スナップ、下絵取込、ピンチズーム/パン UI、スタイル/グラデ/パレット、保存・作品一覧、PNG + SVG エクスポート、仕上げ。下絵・ズームは G より後ろにまとめていた。~~ → 検討メモ（2026-07-13）参照。
...
Phase A〜E 完了時点のコードレビュー（2件）を統合した、**着手前に片付ける修正・設計判断**の正本。→ 検討メモ（2026-07-13）参照。
...
| 5 | **Undo スタック上限** | 無制限 `Artwork` スナップショット。G で数千ポリゴン時にメモリ肥大 → 上限（例 50〜100）+ 切り捨て | **E+** |
...
| 7 | **`weldVertices` figure-8** | 非連続重複（例 `[A,keep,B,keep,C]`）を `_collapseConsecutiveRingIds` が通す。自己接触ポリゴンが G 入力で破綻 → 弾くか正規化 | **E+** |
...
| 15 | **`clearDraft` / `clearAll` の Undo テスト不足** | 実装はあるがテストなし | **E+** |
| 16 | **`detachVertexFromDraft` テスト不足** | polygon 版のみテスト済み | **E+** |
...
- [ ] Undo スタック上限（#5）
- [ ] `weldVertices` figure-8 対策 + テスト（#7）
- [ ] `clearDraft` / `clearAll` の Undo テスト（#15）
- [ ] `detachVertexFromDraft` テスト（#16）
...
- `weldVertices` の非連続重複（figure-8）を弾く/正規化
- `moveVertex` で同座標・別 ID（自動溶接されない）の明示テスト
- `clearDraft` / `clearAll` の Undo 復元
...
※ 自動 E2E は v1.1 検討。v1 は **手動 smoke + `flutter test` CI** でカバー。→ 検討メモ（2026-07-14 追記続き4）参照。
...
- **計画メモ**: Phase 完了時は末尾の検討メモに実装・実機結果を追記してからコミットしてよい。
...
> 経緯・例外の詳細は末尾の検討メモ（2026-07-12）参照。

## 不確定要素とクリア期限

v1 完成直前の停滞を避けるため、以下を「未確定」と明示し **いつまでに決めるか** を Phase ゲートに紐づける。→ 検討メモ（2026-07-14 追記続き4）参照。

| ID | 不確定要素 | リスク | クリア期限（ゲート） | 成果物 |
|----|-----------|--------|---------------------|--------|
| U1 | `ArtworkDocument` v1 スキーマ | Hγ で大規模リファクタ | **G-spike 完了時** | フィールド一覧（紙上可） |
...
- **`weldVertices` figure-8** → G 入力退化（#7）。**E+ で前倒し**。
- Undo メモリ肥大 → 上限 + F/G のバッチ確定（#5）。**E+ で前倒し**。
...
Phase E 完了後のコミット作業で、Phase ごとの細分化が `canvas_provider.dart` 等の共有ファイルに複数 Phase の差分が混在しており困難だった。以下で合意し、本文の「Git / コミット運用」節に反映済み。
...
→ 本文に「品質方針」「v1.0 最小出荷ライン」「コード品質・修正前提」節を追加。frontmatter todos に `phase-e-plus` / `phase-g-spike` / `phase-h-underlay` / `phase-h-viewport` を追加し F/G/H の記述を更新。
...
いずれも「後から入れると作り直しになる」性質の設計判断（実行スレッドモデル・画像取込パイプライン）であり、実装着手前の今のうちに計画へ反映した。既存の決定を覆すものではなく、これまで明示されていなかった要件の追記のため、本文側は運用ルールの「単純な情報追加」に該当すると判断して直接追記した。ただし既存の「完了条件」文は要件が増える分、内容が変わって見えるため ~~取り消し線~~ を引いた旧文を残し、新文を並記した（G-spike／Hα／G の3箇所）。

**追記（2026-07-14 続き4）: 統合・不確定要素・G 三角サイズ（外部レビュー + 内部検討）**
...
**取り消し線を使った箇所**: Phase G の ~~spacing 既定間隔~~ 仕様 → maxEdge/minEdge 仕様（2026-07-14）。#9 着手タイミング列、G 完了条件。
...
→ frontmatter `phase-e-plus` を `completed` に更新。`overview` を「Phase A〜E+ 完了。次は G-spike→…」に更新。次は **Phase G-spike**（`現行サマリ.md` も合わせて更新）。
