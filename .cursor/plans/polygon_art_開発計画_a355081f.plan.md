...
## ポリゴンのクローズ挙動（確定仕様・実装済み）

> 実機テストと相談を重ねて確定した、描画・クローズ操作の統一ルール。
> 実装は [lib/providers/canvas_provider.dart](f:\polygon_art_app\lib\providers\canvas_provider.dart) の
> `handleDrawTap` / `_tryCloseAtVertex`。テストは
> [test/canvas_notifier_test.dart](f:\polygon_art_app\test\canvas_notifier_test.dart)。

### 基本ルール

| 操作 | 挙動 |
|------|------|
| **シングルタップ** | 常に点を追加するだけ（**決して閉じない**）。始点を通過しても閉じずに続行できる＝**一筆書き**に対応 |
| **ダブルタップ（頂点の上）** | その頂点でクローズ（下記2種のみ） |
| **ダブルタップ（空白・描画途中の点の近く）** | クローズせず**点が2つ置かれるだけ**。不要なら Undo で取り消す |

### クローズが成立する2ケースのみ

1. **自己クローズ**（ドラフトの始点の上でダブルタップ）
   - ループとして閉じる。
   - ダブルタップ1打目が始点付近に落とした「捨て点」を除去してから閉じるので、余分な頂点が残らずきれいなループになる（`_lastTapInsertedCount` で1打目の寄与だけを数えて除去）。
2. **連結クローズ**（他ポリゴンの頂点の上でダブルタップ）
   - 1打目でその頂点に**溶接（weld＝既存 Vertex ID を共有）**され、2打目でクローズ。
   - 始点と終点が**同じ**ポリゴンに属する場合は、その共有境界の**短い方の弧**をたどって閉じる（`_sharedBoundaryClosure`）＝隙間なくタイルが繋がる。
   - 判定は「ドラフト終点が確定ポリゴンの頂点か」で行う。`findPolygonVertexNear` はドラフト自身の頂点を除外するため、溶接済みの終点は検出できない。よって**終点方式**で判定する。

### 制約・補助

- クローズには最低 **3 頂点**（`kMinPolygonVertices`）が必要。足りない場合はダブルタップでも閉じず点追加にフォールバック。
- **発見性**: ドラフトが3点以上になったら**始点ドットを強調リング**表示（`polygon_painter.dart`）。ツールバー「閉じる」ボタンもフォールバックとして常設。
- 疑似ダブルタップ判定は `kDoubleTapMaxInterval` / `kDoubleTapMaxDistance`、頂点吸着半径は `kVertexHitRadius`（画面座標。将来のズーム時は scale で world 座標に換算予定）。

### 設計意図・将来との整合

- 「シングル＝追加のみ／ダブル＝頂点上でのみ閉じる」に統一することで、**誤クローズ**（速い手描きや始点通過での勝手な閉じ）を排除しつつ、ルールを単純に保つ。
- 一筆書きで始点を通過して続けた場合、閉じなかった点をそのまま次の描画の起点にできる＝「2匹のヘビが体を共有する」コアの一筆書きデータモデルと整合。
- 将来の自動テッセレーション（丸を描くと内側を三角/四角で自動分割）で生じる**ダングリングな紐（α字の尻尾など）**は、ポリゴン化の際に無視する想定。ここでの「頂点上でのみ閉じる」方針と矛盾しない。
