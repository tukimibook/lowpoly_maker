...
class _PolygonCanvasState extends ConsumerState<PolygonCanvas> {
  ...
  Widget build(BuildContext context) {
    ...
    void updateDrawPreview(Offset localPosition) {
      ...
      // ScaleGestureRecognizer often re-fires onScaleUpdate with an unchanged
      // focal point; skip the O(n) snap search when nothing moved.
      final previous = dragPreview.value;
      if (previous != null && previous.position == position) return;

      final hit = notifier.findPolygonVertexNear(
        position,
        hitRadius: hitRadius(),
      );
      final previousSnap = previous?.snappedVertexId;
